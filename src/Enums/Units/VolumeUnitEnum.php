...
enum VolumeUnitEnum: int implements UnitEnumInterface
{
    ...
    {
        return match ($this) {
            self::MILLILITER => fn (float|int $value): float => (float)$value / 1000,
            self::CENTILITER => fn (float|int $value): float => (float)$value / 100,
            self::DECILITER => fn (float|int $value): float => (float)$value / 10,
            self::LITRE => fn (float|int $value): float => (float)$value,
            self::CUBIC_METER => fn (float|int $value): float => (float)$value * 1000,
            self::TEASPOON => fn (float|int $value): float => (float)$value * 0.00492892,
            self::TABLESPOON => fn (float|int $value): float => (float)$value * 0.0147868,
            self::FLUID_OUNCE => fn (float|int $value): float => (float)$value * 0.0295735,
            self::CUP => fn (float|int $value): float => (float)$value * 0.236588,
            self::PINT => fn (float|int $value): float => (float)$value * 0.473176,
            self::QUART => fn (float|int $value): float => (float)$value * 0.946353,
            self::GALLON => fn (float|int $value): float => (float)$value * 3.78541,
            self::GILL_UK => fn (float|int $value): float => (float)$value * 0.142065,
            self::PINT_UK => fn (float|int $value): float => (float)$value * 0.568261,
            self::QUART_UK => fn (float|int $value): float => (float)$value * 1.13652,
            self::GALLON_UK => fn (float|int $value): float => (float)$value * 4.54609,
            self::CUBIC_INCH => fn (float|int $value): float => (float)$value * 0.0163871,
            self::CUBIC_FOOT => fn (float|int $value): float => (float)$value * 28.3168,
            self::CUBIC_YARD => fn (float|int $value): float => (float)$value * 764.555,
        };
    }
